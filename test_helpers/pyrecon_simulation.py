...
import glob
import sys
from pyrecon import IterativeFFTReconstruction, MultiGridReconstruction
...
if __name__ == '__main__':
    ...
    recon = IterativeFFTReconstruction(f=0.757, bias=2.2, nmesh=512, boxsize=box_size, boxcenter=box_size / 2, los=[0,0,1], wrap=True, nthreads = 128, fft_engine = 'fftw')
    s = time.time()
    recon.assign_data(pre, np.broadcast_to([1.], pre.shape[0]))
    recon.set_density_contrast(smoothing_radius = 15.)
    recon.run(niterations = 3)
    print(f"Iterative recon took {time.time() - s}s", flush=True)
    ...
    print(f"Reading from field took {time.time() - s}s", flush=True)
    ...
    recon = MultiGridReconstruction(f=0.757, bias=2.2, nmesh=512, boxsize=box_size, boxcenter=box_size / 2, los=[0,0,1], wrap=True, nthreads = 128, fft_engine = 'fftw')
    s = time.time()
    recon.assign_data(pre, np.broadcast_to([1.], pre.shape[0]))
    recon.set_density_contrast(smoothing_radius = 15.)
    recon.run()
    print(f"Multigrid recon took {time.time() - s}s", flush=True)
    ...
    fig.savefig("plots/pyrecon_simulation_mg.png")
