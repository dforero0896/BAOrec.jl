...
if __name__ == '__main__':
    ...
    fig.savefig("/home/astro/dforero/codes/BAOrec/examples/simulation_gpu.png")

    

    post_d = np.load("/home/astro/dforero/codes/BAOrec/data/GPU_CATALPTCICz0.466G960S1010008301_zspace.dat.rec.npy").astype(np.double)
    post_r = np.load("/home/astro/dforero/codes/BAOrec/data/GPU_CATALPTCICz0.466G960S1010008301_zspace.ran.rec.iso.npy").astype(np.double)
    ...
    post_r = np.load("/home/astro/dforero/codes//BAOrec/data/GPU_CATALPTCICz0.466G960S1010008301_zspace.ran.rec.sym.npy").astype(np.double)
    post_r = (post_r + 2500) % 2500
    pk = compute_auto_box_rand(post_d[:,0], post_d[:,1], post_d[:,2], np.ones_like(post_d[:,0]),
                      post_r[:,0], post_r[:,1], post_r[:,2], np.ones_like(post_r[:,0]),
                      powspec_conf_file = "examples/powspec_auto.conf",
