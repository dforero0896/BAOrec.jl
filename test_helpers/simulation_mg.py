...
if __name__ == '__main__':
    ...
    post_d = np.load("data/MG_CPU_UNIT_DESI_Shadab_HOD_snap97_ELG_v0.dat.rec.npy").astype(np.double)
    post_r = np.load("data/MG_CPU_UNIT_DESI_Shadab_HOD_snap97_ELG_v0.ran.rec.iso.npy").astype(np.double)
    ...
    post_r = np.load("data/MG_CPU_UNIT_DESI_Shadab_HOD_snap97_ELG_v0.ran.rec.sym.npy").astype(np.double)
    post_r = (post_r + box_size) % box_size
    pk = compute_auto_box_rand(post_d[:,0], post_d[:,1], post_d[:,2], np.ones_like(post_d[:,0]),
                      post_r[:,0], post_r[:,1], post_r[:,2], np.ones_like(post_r[:,0]),
                      powspec_conf_file = "test_helpers/powspec_auto.conf",
    ...
    fig.savefig("plots/simulation_mg.png")