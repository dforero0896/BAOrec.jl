...
if __name__ == '__main__':
    ...
    data = np.load("data/MG_GPU_UNIT_lightcone_multibox_ELG_footprint_nz_NGC.rec.npy").astype(np.double)
    rand = np.load("data/MG_GPU_UNIT_lightcone_multibox_ELG_footprint_nz_1xdata_5.ran_NGC.rec.iso.npy").astype(np.double)
    print(data[:100])
    fkp_data = 1. / (1 + data[:,3] * P0)
    fkp_rand = 1. / (1 + rand[:,3] * P0)
    ...
    rand = np.load("data/MG_GPU_UNIT_lightcone_multibox_ELG_footprint_nz_1xdata_5.ran_NGC.rec.sym.npy").astype(np.double)
    fkp_data = 1. / (1 + data[:,3] * P0)
    fkp_rand = 1. / (1 + rand[:,3] * P0)
    ...
    fig.savefig("plots/lightcone_mg_gpu.png")