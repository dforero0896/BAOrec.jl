...
if __name__ == '__main__':
    ...
    recon = IterativeFFTReconstruction(f=0.757, bias=2.2, nmesh=512, nthreads = 128, fft_engine = 'fftw', boxsize = boxsize, boxcenter = boxcenter, wrap=False)
    s = time.time()
    ...
    fkp_data = 1. / (1 + data[:,3] * P0)
    fkp_rand = 1. / (1 + rand[:,3] * P0)
    recon = MultiGridReconstruction(f=0.757, bias=2.2, nmesh=512, nthreads = 128, fft_engine = 'fftw', boxsize = boxsize, boxcenter = boxcenter, wrap=False)
    s = time.time()
    recon.assign_data(data[:,:3], weights = fkp_data)
    recon.assign_randoms(rand[:,:3], weights = fkp_rand)
    recon.set_density_contrast(smoothing_radius = 15.)
    ...
    fig.savefig("plots/pyrecon_lightcone_mg.png")
